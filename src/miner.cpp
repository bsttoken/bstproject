...
void ThreadStakeMiner(CWallet *pwallet)
{
    ...
    RenameThread("ElephantChain-miner");
...
{
    LogPrintf("ElephantChainMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("ElephantChainMiner-miner");
    ...
    try { while (true) {
        ...
        LogPrintf("Running ElephantChainMiner with %llu transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        LogPrintf("ElephantChainMiner terminated\n");
        throw;
    }
}
