...
#else
    const char* pszModule = "ElephantChain";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ElephantChain
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ElephantChain
    // Mac: ~/Library/Application Support/ElephantChain
    // Unix: ~/.ElephantChain
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ElephantChain";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "ElephantChain.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty ElephantChain.conf if it does not exist
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "ElephantChaind.pid"));
